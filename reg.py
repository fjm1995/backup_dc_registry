...
from impacket.dcerpc.v5.dtypes import READ_CONTROL, NULL
...
class RegHandler:
    ...
    def backup(self, dce, keyName):
        KEY_READ                  = 0x00020019
        REG_OPTION_BACKUP_RESTORE = 0x00000004
        REG_OPTION_OPEN_LINK      = 0x00000008
      
        hRootKey, subKey = self.__strip_root_key(dce, keyName)
        path = self.__options.path + '\\' + subKey
        print('Dumping ' + subKey + ' hive to ' + path) 
        ans2 = rrp.hBaseRegOpenKey(dce, hRootKey, subKey, REG_OPTION_BACKUP_RESTORE | REG_OPTION_OPEN_LINK, KEY_READ)
        ans3 = rrp.hBaseRegSaveKey(dce, ans2['phkResult'], path, NULL)
